...
@web_app.route('/api/profile/<int:telegram_id>', methods=['GET', 'POST'])
def handle_profile(telegram_id):
    ...
    if request.method == 'GET':
        ...
        c.execute('''INSERT OR REPLACE INTO user_profiles 
                     (telegram_id, username, display_name, created_at)
                     VALUES (?, ?, ?, ?)''',
                  (telegram_id, data.get('username', ''), data.get('display_name', ''), datetime.now()))
        conn.commit()
        conn.close()
...
def handle_playlists(telegram_id):
    ...
    if request.method == 'GET':
        c.execute('''SELECT id, name, created_at FROM playlists WHERE telegram_id = ? ORDER BY created_at DESC''', (telegram_id,))
        ...
        data = request.json
        c.execute('''INSERT INTO playlists (telegram_id, name, created_at) VALUES (?, ?, ?)''',
                  (telegram_id, data.get('name'), datetime.now()))
        conn.commit()
...
def handle_playlist_tracks(playlist_id):
    ...
    if request.method == 'GET':
        c.execute('''SELECT track_id, track_title, track_artist, track_url, track_thumbnail, position
...
# ===== API ИСТОРИИ =====
@web_app.route('/api/history/<int:telegram_id>', methods=['POST'])
def add_to_history(telegram_id):
    data = request.json
    conn = sqlite3.connect('music_bot.db')
    c = conn.cursor()
